...
export default function Profile() {
  ...
  const [phone, setPhone] = useState("+998 xx xxx xx xx");
  ...
  const [tempPhone, setTempPhone] = useState("+998 xx xxx xx xx");
  const [isEditing, setIsEditing] = useState(false);

  const fileInputRef = useRef<HTMLInputElement>(null);

  // Проверка хранилища и истечения времени верификации
  useEffect(() => {
  ...
  // Функция для ввода телефона (принимает только цифры и знак '+')
  const handlePhoneChange = (e: ChangeEvent<HTMLInputElement>) => {
    const onlyNumbers = e.target.value.replace(/[^0-9+]/g, "");
    setTempPhone(onlyNumbers);
  };

  // Функция фоновой отправки уведомлений в Telegram-бот (через API)
  ...
  const handleSave = () => {
    ...
    // Подготовка текста уведомления для Telegram (на русском языке)
    let telegramMessage = `🔔 *TrendTik | Профиль обновлен!*\n\n`;
    telegramMessage += `👤 *Имя:* ${name}\n`;
    telegramMessage += `👥 *Фамилия:* ${surname}\n`;
    telegramMessage += `📞 *Телефон:* \`${updatedPhone}\`\n`;

    // ПРОВЕРКА УСЛОВИЯ: Если введен верный промокод для TrendTik
    if (promoCode === "TRENDTIK" || promoCode === "TRENDTIK") {
      setIsVerified(true);
      ...
      setTimeLeft("Осталось: 24ч 0м");

      telegramMessage += `\n🔑 *VIP Статус:* Пользователь ввел промокод, аккаунт успешно верифицирован! ✅`;
    } else if (promoCode.trim() !== "") {
      alert("⚠️ Неверный промокод! Возьмите рабочий код в нашем Telegram-канале.");
      telegramMessage += `\n❌ *Ошибка:* Попытка ввода неверного промокода: \`${promoCode}\``;
    }
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 my-6">
        <div className="bg-zinc-900/40 p-8 rounded-3xl border border-zinc-800/80 max-w-md w-full shadow-2xl backdrop-blur-xl relative">
          ...
          <div className="text-center mb-6 flex flex-col items-center relative z-10">
            ...
            {/* Обновленный дизайн подтверждения аккаунта с синей галочкой */}
            {isVerified && (
          ...
          <div className="space-y-4 relative z-10">
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                {isPhoneLocked && <span className="text-[10px] text-amber-500 font-semibold bg-amber-500/10 px-2 py-0.5 rounded">🔒 Изменение недоступно</span>}
              </div>
              {isEditing && !isPhoneLocked ? (
                <div>
                  ...
                  />
                  <p className="text-[10px] text-zinc-500 mt-1">⚠️ Вы можете изменить номер только 1 раз! Разрешены только цифры.</p>
                </div>
              ) : (
            ...
            {isEditing && !isVerified && (
              <div className="pt-2 border-t border-zinc-800/60">
                <label className="text-xs text-sky-400 uppercase font-bold tracking-wider block mb-1">Промокод из Telegram 🔑</label>
                <input 
                  type="text" 
